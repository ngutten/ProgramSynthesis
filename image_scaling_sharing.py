...
class Operators(nn.Module):
    def __init__(self, N):
        ...
        self.optim = torch.optim.Adam(self.parameters(), lr=1e-4) #[p for o in operators for p in o.parameters()], lr=1e-4)
    ...
    def forward(self, zz, seq, dummy, detach=False):
        ...
        return torch.sum(F.softmax(seq,dim=1).view(seq.size(0), 1, 1, 1, len(self.operators)) * allzs, 4)  
...
def attempt_exchange(i):
    ...
    newprog1.programs.data[:,:,-1] = avg_logit.data.clone() - 1 # Make the new operator a little weaker to avoid immediate interference
...
for epoch in range(1501):
    ...
    if epoch%20 == 0:
        ...
        np.savetxt("%s/exchanges.txt" % (data_dir), 
            np.concatenate([np.array(exchange_epochs)[:, np.newaxis], 
            np.array(exchanges)[:,np.newaxis],
            np.array(deletions)[:,np.newaxis], 
            np.array(opcount)[:,np.newaxis]], axis=1))
    lstats = []
    i = 0
